...
The group-theoretic (co)homology of $\Gamma$ is defined via the symmetries of proper discontinuous actions $\Gamma \times X \to X$ on $E\Gamma$ models $X$. There is extensive literature on this subject, c.f.\cite{Brown}. The standard $E\Gamma$ model for the mapping class group is Teichmueller's $X=\sT_g$, a topological $(6g-6)$-cell $\simeq \bR^{6g-6}$, e.g. \cite{hubbard}. %There are alternative models which are also worth exploring, as will be discussed below \S \ref{s2}. 
